/*
 * $Id: Arc.c,v 1.1.1.1 2004/08/09 20:03:03 liao Exp $
 *
 * $Log: Arc.c,v $
 ...
 *
 * Revision 1.6  2000/02/18 18:57:52  antool
 * fix XtFree bug that cause segmentation fault. by weng.
 *
 * Revision 1.5  1997/08/12 13:38:28  markus
 * just a few minor improvements
 *
 * Revision 1.4  1997/02/10 13:52:51  markus
 * added a patch
 *
 * Revision 1.3  1996/06/27 18:23:08  bob
 * Changed #if ((XmREVISION==2) || (XmVERSION==2)).
 * Changed #include <Xm/Xm.h>.
 * Inserted #ifdef LINUX...
 *
 * Revision 1.2  1996/06/14 21:40:53  markus
 * just a few cp's away
 *
 * Revision 1.1  1996/02/20 09:57:11  martin
 * Initial revision
 *
 */
...
* File:         Arc.c
* RCS:
* Description:  An Arc widget for use with the GRAPH widget.
...
{
    XmNto, XmCTo, XmRPointer, sizeof(XtPointer),
    XtOffset(XmArcWidget, arc.to), XmRPointer, NULL
    },

{
    XmNfrom, XmCFrom, XmRPointer, sizeof(XtPointer),
...

{
    ...
    XtOffset(XmArcWidget, arc.highlightcolor), XmRString, "DarkOrange"
    },
...
{
    XmNkeephidden,  XmCKeephidden, XmRBoolean, sizeof(Boolean),
    XtOffset(XmArcWidget, arc.keephidden), XmRString, "False"
    },
...
{
    XmNarmCallback, XmCCallback, XmRCallback, sizeof(XtPointer),
    XtOffset(XmArcWidget, arc.arm_callback), XmRPointer, (XtPointer) NULL
    },

{
    XmNdisarmCallback, XmCCallback, XmRCallback, sizeof(XtPointer),
    XtOffset(XmArcWidget, arc.disarm_callback), XmRPointer, (XtPointer) NULL
    },

{
    XmNactivateCallback, XmCCallback, XmRCallback, sizeof(XtPointer),
    XtOffset(XmArcWidget, arc.activate_callback), XmRPointer, (XtPointer) NULL
    },

{
    XmNarcEditedCallback, XmCCallback, XmRCallback, sizeof(XtPointer),
    XtOffset(XmArcWidget, arc.arc_edited), XmRPointer, (XtPointer) NULL
    },
{
    XmNuserData, XmCUserData, XmRPointer, sizeof(XtPointer),
    XtOffset(XmArcWidget, arc.user_data), XmRPointer, (XtPointer) NULL
    }
};
...
{ XmArcWidget new =  (XmArcWidget) new_w;
  ...
  new->arc.keephidden = FALSE;
  new->arc.visible    = FALSE;
  new->arc.armed      = FALSE;

/* ME : wasn 't initialized :*/
  new->arc.map_name=FALSE;
...
/* _XmGetDefaultFontList() is not implemented in _R3_ Motif1.0 */
  ...
  if(new->arc.label == NULL)
  {
    ...
    /* changed by weng to fix XtFree bug that cause segment.fault */
    XmStringFree(string);
  }
  else
  {
	...
	else
	{
	    ...
	    string   =  XmStringLtoRCreate(XrmQuarkToString(new->core.xrm_name), cset);
	    new->arc.label =  _XmStringCreate(string);
          /* changed by weng to fix XtFree bug that cause segmemt.fault*/
	    XmStringFree(string);
...
*/
	...
	case 0:
	  ...
	  name_x = x + width/2;
#if SESD
/* ME :  - arc->arc.labelheight /4  is new */
	  name_y = y - ((arc->arc.labelheight)/4);
#else
	...
	case 2:
	  ...
	  name_x = x + width/2;
#if SESD
/* ME : 01021995 : it was  : name_y = y + height; */
	  name_y = y + height + ((arc->arc.labelheight)/4);
#else
...
#endif
{
  ...
  if(!to || !from || !XtIsRealized(parent) || (w->arc.keephidden) )
    return;
...
*/
		...
		name_x = x + width/2;
#if SESD
/* ME :  - (w->arc.labelheight / 4) is new */
		name_y = y - ((w->arc.labelheight)/4);
#else
...
#if SESD
/* ME : 01021995 : it was  : name_y = y + height; */
		name_y = y + height + ((w->arc.labelheight)/4);
#else
...
#endif
{
  ...
  if (new->arc.label != current->arc.label)
  {
    ...
    if (new->arc.label == NULL)
    {
      ...
      new->arc.label =  _XmStringCreate(string);
    /* changed by weng to fix XtFree that cause segment.fault */
      XmStringFree(string);
    }
    else
    {
	...
	string = XmStringLtoRCreate(XrmQuarkToString(new->core.xrm_name), cset);
	new->arc.label =  _XmStringCreate(string);
      /* changed by weng XtFree that cause segment.fault */
	XmStringFree(string);
      }
    }
...
/* ME : mapLabel was never set !! */
  if (current->arc.map_name != new->arc.map_name)
  {
    if (current->arc.map_name)
      new->arc.map_name=0;
    else
      new->arc.map_name=1;
  }
...
/* ME: I changed the MAX_ARCS from the original 200 to 2000 */
#define MAX_ARCS 2000
static struct  {
  int       n;
...
void XmArcKeepHidden
#ifndef _NO_PROTO
  (XmArcWidget w,Boolean value) /* ME : if true : hide arcs always */
#else
  (w,value)
  XmArcWidget w;
  Boolean value;
