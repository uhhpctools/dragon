/*
 * $Id: Graph.c,v 1.1.1.1 2004/08/09 20:03:03 liao Exp $
 *
 * $Log: Graph.c,v $
 * Revision 1.1.1.1  2004/08/09 20:03:03  liao
 * Imported Dragon from Lurch
 *
 * Revision 1.1.1.1  2003/06/13 21:28:14  sgicomp
 * The Dragon Analysis Tool
 *
 * Revision 1.1.1.1  2002/09/10 22:45:19  sgicomp
 * Create the first version of the Dragon
 *
 * Revision 1.6  1997/08/12 13:38:56  markus
 * just a few minor improvements like setting the maxnumber of rectangles
 * higher than 200.
 *
 * Revision 1.5  1997/02/10 15:04:52  markus
 * corrected error with "X11/cursorfont.h" -> <X11/cursorfont.h>
 *
 * Revision 1.4  1996/06/27 19:06:43  bob
 * Fixed typos.
 *
 * Revision 1.3  1996/06/27 18:42:00  bob
 * Substituted _XmInputInGadget() with XmObjectAtPoint().
 *
 * Revision 1.2  1996/06/14 21:40:31  markus
 * included Xm/ScrollBar.h
 *
 * Revision 1.1  1996/02/20 09:57:28  martin
 * Initial revision
 *
 */
...
*
* File:         Graph.c
* RCS:          $Header: /usr/local/cvs/dragon/XmGraph/Graph.c,v 1.1.1.1 2004/08/09 20:03:03 liao Exp $
...
/* ME : added for LINUX because there were complaints about undeclared
XmARM_EVENT,XmLEAVE_EVENT,XmENTER_EVENT,XmACTIVATE_EVENT
*/
...
#ifdef _R4_
#error "What a pity - it's R4 !"
#endif
*/

#if (XmREVISION > 1) || (XmVERSION==2)
#define RX XtX
#define RY XtY
 ...
 */
/* ME: I changed the MAX_LINES from the original 200 to 2000 */
...
/* ME: I changed the MAX_RECTANGLES from the original 200 to 2000 */
#define MAX_RECTANGLES 2000
...
static Widget FindWidgetatXY(Widget fw, int x, int y);
...
{XmNptrCursor, XmCCursor, XmRCursor, sizeof(Cursor),
     XtOffset(XmGraphWidget, graph.ptr_cursor), XmRString, (XtPointer) "left_ptr"},

{XmNmotionCursor, XmCCursor, XmRCursor, sizeof(Cursor),
     XtOffset(XmGraphWidget, graph.motion_cursor), XmRString, (XtPointer) "fleur"},

{XmNindicateCursor, XmCCursor, XmRCursor, sizeof(Cursor),
     XtOffset(XmGraphWidget, graph.indicate_cursor), XmRString, (XtPointer) "crosshair"},

{XmNindicateChildCursor, XmCCursor, XmRCursor, sizeof(Cursor),
     XtOffset(XmGraphWidget, graph.indicate_child_cursor), XmRImmediate, (XtPointer) None},

{XmNindicateParentCursor, XmCCursor, XmRCursor, sizeof(Cursor),
     XtOffset(XmGraphWidget, graph.indicate_parent_cursor), XmRImmediate, (XtPointer) None},

{ XmNdoubleClickInterval, XmCDoubleClickInterval, XmRInt, sizeof(int),
      XtOffset(XmGraphWidget, graph.double_click_interval), XmRImmediate,  (XtPointer) 250  },
...
{ XmNorientation, XmCOrientation, XmROrientation, sizeof(unsigned char),
      XtOffset(XmGraphWidget, graph.direction),  XmRImmediate, (XtPointer) XmHORIZONTAL},
...
{XmNnewArcCallback, XmCCallback, XmRCallback, sizeof(XtPointer),
     XtOffset(XmGraphWidget, graph.new_arc), XmRPointer, (XtPointer) NULL},
 
{XmNnewNodeCallback, XmCCallback, XmRCallback, sizeof(XtPointer),
     XtOffset(XmGraphWidget, graph.new_node), XmRPointer, (XtPointer) NULL},
 
{XmNnodeMovedCallback, XmCCallback, XmRCallback, sizeof(XtPointer),
     XtOffset(XmGraphWidget, graph.node_moved), XmRPointer, (XtPointer) NULL},
 
{XmNarcMovedCallback, XmCCallback, XmRCallback, sizeof(XtPointer),
     XtOffset(XmGraphWidget, graph.arc_moved), XmRPointer, (XtPointer) NULL},
 
{XmNlayoutProc, XmCLayoutProc, XmRPointer, sizeof(XtPointer),
     XtOffset(XmGraphWidget, graph.layout_proc), XmRImmediate, (XtPointer) DoLayout},
 
{XmNdefaultActionCallback, XmCCallback, XmRCallback, sizeof(XtPointer),
     XtOffset(XmGraphWidget, graph.double_click), XmRPointer, (XtPointer) NULL},
 
{XmNselectNodeCallback, XmCCallback, XmRCallback, sizeof(XtPointer),
     XtOffset(XmGraphWidget, graph.select_node), XmRPointer, (XtPointer) NULL},
 
{XmNdeselectCallback, XmCCallback, XmRCallback, sizeof(XtPointer),
     XtOffset(XmGraphWidget, graph.deselect), XmRPointer, (XtPointer) NULL},
 
{XmNselectArcCallback, XmCCallback, XmRCallback, sizeof(XtPointer),
     XtOffset(XmGraphWidget, graph.select_arc), XmRPointer, (XtPointer) NULL},
 
{XmNselectSubgraphCallback, XmCCallback, XmRCallback, sizeof(XtPointer),
     XtOffset(XmGraphWidget, graph.select_subgraph), XmRPointer, (XtPointer) NULL},
 
{XmNdefaultNodeClass, XmCDefaultNodeClass, XmRInt, sizeof (int),
     XtOffset(XmGraphWidget, graph.default_widget_class), XmRInt, (XtPointer) &resource_0_int},
 
{XmNinitializeDataCallback, XmCCallback, XmRCallback, sizeof(XtPointer),
     XtOffset(XmGraphWidget, graph.constraint_init), XmRPointer, (XtPointer) NULL},
 
{XmNfreeDataCallback, XmCCallback, XmRCallback, sizeof(XtPointer),
     XtOffset(XmGraphWidget, graph.constraint_free), XmRPointer, (XtPointer) NULL},
 
{XmNsnapGridOn, XmCSnapGridOn, XmRBoolean, sizeof (Boolean),
     XtOffset(XmGraphWidget, graph.snap_on), XmRImmediate, (XtPointer) FALSE },
 
{XmNsnapGridSize, XmCSnapGridSize, XtRCardinal, sizeof (Cardinal),
     XtOffset(XmGraphWidget, graph.snap_grid_size), XmRImmediate, (XtPointer) 10 },
 
{XmNinteractiveArcDirection, XmCInteractiveArcDirection, XmRArcDirection, sizeof (unsigned char),
     XtOffset(XmGraphWidget, graph.arc_style), XmRImmediate, (XtPointer) XmUNDIRECTED },
...
static Widget FindWidgetatXY(Widget fw, int x, int y)
#else
static Widget FindWidgetatXY(fw, x, y)
Widget fw;
int x;
int y;
#endif
...
/* ME : in Motif 2.0 , X11R6 on LINUX the procedure
_XmInputInGadget() didn't exist.
So we substituted it with XmObjectAtPoint() which seems to do the same
*/

#ifndef LINUX
  return_wid = (Widget) _XmInputInGadget(fw,x,y);
#else
  return_wid = (Widget) _XmInputInGadget(fw,x,y);
  //  return_wid = XmObjectAtPoint(fw, (Position)x,(Position)y);
#endif
  return return_wid;
}
...
#endif
{
    ...
    if (((request->request_mode) & XtCWQueryOnly) == 0) /* CYY  */
 ...
 */  
  ...
  /* ME 8.11.1994 :
	 If height was between 30000 and 32000 the graph widget crashed
	 with
	 X Error of failed request:
	       BadValue (integer parameter out of range for operation)
	 Major opcode of failed request:  12 (X_ConfigureWindow)
	 Value in failed request:  0x12000b0

	 !!!
	 Therefore limit height to 30000 which is quite large on a normal screen.
	 You can only see one node maximum with that size !
       */
  if (min_height >= 30000)
  {
    graph->graph.deferedAdjustSize = TRUE;
  ...
  /* ME 8.11.1994 : prohibit heights larger than 30000 because 
     XmGraph crashes afterwards
   */
  if (min_width >= 30000)
  {
    graph->graph.deferedAdjustSize = TRUE;
...
#if SESD
  if (min_width > RWidth(graph) ||
      min_height > RHeight(graph) ||
      min_width +50 < RWidth(graph) ||
      min_height+50 < RHeight(graph)) 
#else
    if (min_width > RWidth(graph) ||
	min_height > RHeight(graph)) 
#endif
...
#ifdef TRACING
      printf("quested:Width=%d,height=%d\n",min_width,min_height);
#endif
    ...
    if (min_width  + 50 < RWidth(graph) || min_height +  50 < RHeight(graph)) 
    {
      result = XtMakeResizeRequest (graphW, min_width, min_height,
				    &replyWidth, &replyHeight);
	...
	/* ME: you can't rely on that : check if it's really a scrolled window !! */
	if (!(XtIsSubclass(swindow,xmScrolledWindowWidgetClass)))
	{
	  swindow = XtParent(graph);
	}
	...
	if(vscroll)
	{
	  ...
	  XmScrollBarSetValues (vscroll, value, slider_size, SCROLLBAR_INCREMENT,
				page_increment, TRUE);
	}
	if(hscroll)
	{
...
#else
void XmGraphMoveAll
  (graphW, delta_x, delta_y) Widget 	graphW; 
		int           delta_x, delta_y;
#endif
{ XmGraphWidget graph = (XmGraphWidget) graphW;
    XmArcWidgetList   arcs;
...
{ Widget	graphW = (Widget)	graph;
  ...
  int n_arcs = graph->graph.n_arcs;
  Widget arcW;
  XEvent *expevent; /* new by ME 9.9.1993 */
	...
	if (!region) 
	{
	    ...
	    if ((event) &&
		(XtIsComposite(graphW)))
	    {
	      /* ME : Redisplay needs CompositeWidg */

	      expevent = event;   /* new by ME 9.9.1993 */
	      
	      _XmRedisplayGadgets ((Widget)graphW,
				   expevent,
				   NULL);
	    }
...
#endif
{
	...
	if (graph->graph.n_arcs == graph->graph.n_arc_slots) 
	{
	    ...
	    children = (WidgetList) XtRealloc((XtPointer) children,
...
#if (XmREVISION==1)
      (w =  FindWidgetatXY ((CompositeWidget)graph, 
			    event->xbutton.x,
			    event->xbutton.y)) != NULL)
#else
      (w =  FindWidgetatXY ((Widget)graph, 
			    event->xbutton.x,
			    event->xbutton.y)) != NULL)
#endif
    { 
...
#if (XmREVISION==1)
    if( (w =  FindWidgetatXY ((CompositeWidget)graph,
			      event->xbutton.x,
			      event->xbutton.y)) != NULL) 
#else
    if( (w =  FindWidgetatXY ((Widget)graph,
			      event->xbutton.x,
			      event->xbutton.y)) != NULL) 
#endif
    {
...
#endif
{
  ...
  if ( graph->graph.edit_mode)
  {
    ...
    if (event->xbutton.subwindow || w != NULL)
    { 
      if(event->xbutton.subwindow)
	w = XtWindowToWidget (XtDisplay(graph),
			      event->xbutton.subwindow);
	...
	if(XtCallbackHasSome == XtHasCallbacks(graphW, XmNdeselectCallback))
	{
	  ...
	  cb.selected_widgets = XmGraphGetSelectedNodes (graphW,
							 &cb.num_selected_widgets);

	  cb.selected_arcs = XmGraphGetSelectedArcs (graphW,
						     &cb.num_selected_arcs);
...
/* ME: sometimes the line_list.n is a number higher than 2000
   I couldn't find the reason for that so I reset it to 0 here
   because this seems to be the correct initial value */
  ...
  if ( graph->graph.edit_mode) 
  {
    ...
    if( (arcW =  XmGraphInputOverArc(graphW, event->xbutton.x,
				     event->xbutton.y)) != NULL) 
    { 
#ifdef __sun
	...
	XDefineCursor (XtDisplay (graph), XtWindow (graph),
	 ...
	 of the arc around. */
      values.line_width = 2;
      XChangeGC(XtDisplay(graph), graph->graph.xorgc, GCLineWidth,
		&values);
...
{ Widget graphW = (Widget)graph;
	...
	else if (graph->graph.current_action == ADDING_ARC)
	{
	    ...
	    if (event->xbutton.subwindow  || 
#if (XmREVISION==1)
		(g = (XmGadget)FindWidgetatXY ((CompositeWidget)graph,
					       event->xbutton.x, 
					       event->xbutton.y)) != NULL)
#else
		(g = (XmGadget)FindWidgetatXY ((Widget)graph,
					       event->xbutton.x, 
					       event->xbutton.y)) != NULL)
#endif
...
{ 
    ...
    int        i, n_nodes = (node ? node->kids.n_nodes : 0);
    Widget     w = (node ? node->widget : NULL);
...
{ 
	...
	alist->arcs = arcs =
	    (XmArcWidgetList) XtRealloc ((XtPointer) arcs,
...
#ifndef _NO_PROTO
  ( Widget scrollbar, 
			    XtPointer graphPtr, XtPointer call_data)
#else
...
{
    char * in_str = strdup(from_val->addr);
...
#endif
{
  if (w && XtIsSensitive (w) && XtIsManaged (w))
    (*(((XmArcWidgetClass) (w->core.widget_class))->arc_class.input_dispatch));
/* (w , maske, event); */
}
...
#endif
{
	...
	nlist->nodes = 
	    (NList) XtRealloc ((XtPointer) nlist->nodes,
...
{ XmGraphWidget graph = (XmGraphWidget) graphW;
    if(graph->graph.is_scrolled)
    {
	static XmScrollBarCallbackStruct cdata =  { XmCR_DRAG, NULL, 0, 0};
...
/* ME: you can't rely on that : check if it's really a scrolled window !! */
	 ...
	 */
	if ((vscroll)
	    && XtIsManaged(vscroll)) /* ME : new because failure  */
	{
	    XmScrollBarGetValues (vscroll,
				  &value, 
				  &slider_size,
				  &increment,
				  &page_increment);
...
#ifdef TRACING
      printf("max=%d,min=%d,increment=%d,height=%d,slisize=%d=>\ncenteratpos y= %d,\n",
		   max,min,increment,XtHeight(graph),slider_size,y);
#endif
	    XmScrollBarSetValues (vscroll, y, slider_size, increment, page_increment,
				  TRUE);
	    /*
	     * Hack due to Motif deficiency.
	     */
...
#endif
{
  ...
  int limit = 4096; /* 910318 mohammad - changed this to a variable to 
		       ...
		       2 ** 10 = 4096 
                     */
  /* Screen *graphscreen;  ME : 8.11.1994 */
  /* int scrheight;        ME : 8.11.1994 */
  /* int scrwidth;         ME : 8.11.1994 */


  /* graphscreen=XtScreen(graph);            ME : 8.11.1994 */
  /* scrheight=HeightOfScreen(graphscreen);  ME : 8.11.1994 */
  /* scrwidth=WidthOfScreen(graphscreen);    ME : 8.11.1994 */ 
   
#ifdef TRACING
  printf("ZOOM: FACTOR|%f -- CHILD SPACE|%d -- SIBLING SPACE|%d,Max:%f\n", 
	 factor, graph->graph.child_spacing, graph->graph.sibling_spacing,
	 (limit && factor));
#endif
   

  if (graph->graph.child_spacing > limit && factor > 0)
    /* Crude check to prevent rediculous zooming */
    return;
...
#ifdef TRACING
  printf("XmGraphCenterValues : %d,%d\n",
	 (int) (xper * RWidth(graph)),
	 (int) (yper * RHeight(graph)));

#endif

  XmGraphCenterAtPos(graphW,
		     (int) (xper * RWidth(graph)),
		     (int) (yper * RHeight(graph)));
}
...
/* ME : new 9.1.1995 */
void XmGraphSetScrolled
#ifndef _NO_PROTO
(Widget w,int bool)
#else
(w,bool) Widget w;int bool;
#endif
{
  XmGraphWidget graphW = (XmGraphWidget) w;

  graphW->graph.is_scrolled = bool;
}
...
{ XmGraphWidget graph = (XmGraphWidget) widget;
  ...
  if (!XtIsSubclass(widget,xmGraphWidgetClass))  return 0;
...
extern void XfwfResizeThumb_Interface(Widget parm0, double parm1, double parm2)
{
 XfwfResizeThumb(parm0, parm1, parm2);
}

extern void XfwfGetThumb_Interface(Widget parm0, XfwfScrollInfo *parm1)
{
  XfwfGetThumb(parm0, parm1);
}
extern void XfwfMoveThumb_Interface(Widget parm0, double parm1, double parm2)
{
  XfwfMoveThumb(parm0,parm1,parm2);   
}
#endif
